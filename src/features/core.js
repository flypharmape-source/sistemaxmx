...
/* ---------- Pendências (central do líder) ---------- */
export const STATUS_PEND = {
  pendente:  { label: "Pendente",  bg: "#FBF1DA", ink: "#8A6410" },
  atrasada:  { label: "Atrasada",  bg: "#FAE7E4", ink: "#A83226" },
  concluida: { label: "Concluída", bg: "#E3F2E6", ink: "#256B3B" },
};
export function statusPendencia(p) {
  if (p.status === "concluida") return "concluida";
  if (p.prazo && p.prazo < HOJE) return "atrasada";
  return "pendente";
}
export const seedPendencias = [
  { id: "pd1", tipo: "escala_confirmar", titulo: "Confirmar quem trabalhou", sub: "Fim de semana 21–22/06 · equipe Tráfego", responsavelId: "c1", setor: "Tráfego", refId: null, competencia: "2026-06", prazo: "2026-06-27", status: "pendente" },
  { id: "pd2", tipo: "escala_cadastrar", titulo: "Cadastrar escala da equipe", sub: "Feriado Revolução Constitucionalista · 09/07", responsavelId: "c1", setor: "Tráfego", refId: "h1", competencia: "2026-07", prazo: "2026-07-02", status: "pendente" },
];
...
export const LEMBRETE = {
  pendencia:    { icon: Bell,          cor: "#A83226", prio: 0 },
...
export function gerarLembretes({ colaboradores, ferias, notas, pagamentos, documentos, pendencias = [] }) {
  const L = [];
  pendencias.forEach((p) => {
    const st = statusPendencia(p);
    if (st !== "concluida") L.push({ id: "pd" + p.id, tipo: "pendencia", titulo: (st === "atrasada" ? "Atrasada: " : "") + p.titulo, sub: p.sub, colaboradorId: p.responsavelId });
  });
